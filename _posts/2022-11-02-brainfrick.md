...
A brainfuck program works by manipulating a "tape" of memory cells. This might sound awfully familiar if you've heard of a [Turing Machine](https://en.wikipedia.org/wiki/Turing_machine). 
...
Maybe don't submit transpiled brainfuck as your first C assignment. 
...
To write our brainfuck interpreter, we will "compile" our brainfuck source into equivalent instructions that we will then run on our brainfuck Virtual Machine.
...
First, we define our `Instructions`. Notice we only define 6 main instructions, we combined `>` with `<` and `+` with `-` since they are just negated versions of each other. Later on we will see the advantage to doing so.
...
pub fn compile(src: &str) -> Program {
    ...
    // we could use a map here as well, but we opt for a vector for increased performance at the cost of using a flat `usize` more memory per instruction
...
Now we define a `run` function to execute our brainfuck code. We will iterate through our instructions, stopping once we run out.
    ...
    pub fn run(mut self) {
        ...
        while instruction_ptr < self.program.instructions.len() {
            ...
            match instruction {
                ...
                In => self.data[self.ptr] = std::io::stdin()
                    .bytes()
                    .next().unwrap().unwrap(),
                Loop => {
...
We start a loop that we will run until our `instruction_ptr` reaches the end of our program. Then, we grab the `instruction` at `instruction_ptr`, and use `match` to find the implementation. 

If you have a keen eye you'll notice some weird choices right away. For example, why bother making `>` and `<` into the same `Shift(isize)` instruction if we have to branch to handle it anyway...
...
Brainfuck only lets us increment/shift by one at a time, but at runtime, there's no reason for us to pay this cost. We can 'contract' repeated instructions into a single instruction with an `O(1)` cost.

This applies to repeated `>`/`<` and `+`/`-` instructions. This is why we defined them together in our `Instruction` enum, so that we can easily contract them together. 
...
Hmm, so this chunk decrements the current cell until it reaches 0. Couldn't we just set the value of the cell to 0? Exactly!
...
If we ever encounter `Loop, Alt(-1), End`, we just remove those 3 instructions from the output, and substitute `Clear`.

Now in our VM, we add a branch to support our new instruction
...
There are plenty of other optimization you can make, my compiler implements one more that replaces certain move/copy/multiply/add operations with some new instructions, but it's not quite as simple as the others. I'll leave some brainfuck below and you can think about how you might try to optimize them with the same strategy. (Hint: try transpiling them to a language you like and applying our previous optimizations)
...
Let's try timing the runtime with and without optimizations to see the impact. Unfortunately it isn't quite fast enough to make use of our rust benchmark framework, [Criterion](https://github.com/bheisler/criterion.rs), so we will just use our shell utility `time` instead.
...
I'm sure these times would be really impressive to a computer scientist in the 60's.
